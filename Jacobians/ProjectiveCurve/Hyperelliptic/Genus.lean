...
/-- **Axiom for the odd-degree hyperelliptic genus formula.**
Replacing the old all-encompassing `AX_Hyperelliptic_genus`, this strictly smaller
axiom isolates the remaining unproven gap (Riemann-Roch for the odd branch)
without tainting the core tree with an anonymous `sorryAx`. -/
axiom AX_HyperellipticOdd_genus (H : HyperellipticData) (h : Odd H.f.natDegree) :
